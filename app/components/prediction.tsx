...
export default function Prediction({ formId, featureNames }: PredictionProps) {
  ...
  return (
    <div className="mt-8">
      <div className="text-xl font-semibold mb-2 self-start">Make a Prediction</div>
      {featureNames.map(featureName => (
        <div key={featureName} className="mb-3">
          <label htmlFor={featureName} className="block mb-1">{featureName}:</label>
          <input
            type="number"
            min="0"
            step="3"
      ...
      <Link href={`/update/${formId}`}><div className='underline text-red-500'>update model</div></Link>
      </div>
      ...
      {prediction !== null && (
        <div className="mt-6 border flex flex-col gap-4 border-gray-200 rounded py-4 px-6">
          <div className=" self-start text-lg  text-gray-600 underline">Response</div>
          <div className='text-black px-2 py-1 rounded-md bg-gray-200'>{prediction}</div>
        </div>
      )}
    </div>
  );
