...
const Nav = () => {
  ...
  return (
    <nav className={`${isOpen && isMobile ? "bg-blackt" : "bg-white"} transition-colors duration-300`}>
          ...
          <div className="flex items-center">
            <Link href="/">
              <div className="flex items-center gap-2 font-bold">
                <Image
                  src={isOpen && isMobile ? "/plutoflow2.png" : "/plutoflow.png"}
                  alt="PlutoFlow logo"
                  width={45}
                  height={45}
                />
              ...
              <div className="flex gap-4 bg-blue-900 text-white py-2 px-4 rounded-l-full">
                {menuItems.map((item, index) => (
                  <Link key={index} href={item.url}>
                    <div className="hover:text-gray-300 transition-colors duration-300">{item.text}</div>
                  </Link>
                ))}
              </div>
